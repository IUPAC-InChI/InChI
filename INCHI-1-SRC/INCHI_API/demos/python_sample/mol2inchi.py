    ...
    if inchi_result <= 1:
        ...
        if calc_key == 1:
            if ikey_result != 0:
                ...
                if ikey_result == 11:
                    flog.write(
                        "Warning: InChI reversal gets mismatching InChI string\n"
                    )
...
def cl_parser():
    ...
    parser.add_option(
        ...
        "--xtra",
        dest="xtra",
        help="calculate extra hash complementing InChIKey",
        default=False,
        action="store_true",
    )
    parser.add_option(
...
for record in content:
    ...
    results = inchify_molfile(
        record, inchi_options, show_auxinfo, calc_key, calc_xkey, inchi_api
    )
