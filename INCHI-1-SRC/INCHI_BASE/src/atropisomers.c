    ...
    for (int i = 0; i < num_atoms; i++) {
        ...
        if (num_neighbors_i == 3) {
            for (int j = 0; j < num_neighbors_i; j++) {
                ...
                score -= (nof_wedge_bonds_i == 0) * 2;
                score -= (nof_wedge_bonds_j == 0) * 2;
                ...
                score -= (atom_in_same_small_ring == 1) * 10;

                if (score > 10) {
                    printf(">>> potential atropisomer: atom id %2d atom id %2d --> score %2d (%d)\n", atom_id1, atom_id2, score, atom_in_same_small_ring);
                    orig_inp_data->is_atropisomer = 1;
